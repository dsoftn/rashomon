...
class RashomonGUI(QMainWindow):
    ...
    def tree_seg_current_changed(self, x, y):
    ...
    def txt_find_text_changed(self):
        ...
        cf.setBackground(color)
        cur.setPosition(0)
        cur.movePosition(cur.Right, cur.KeepAnchor, len(result_txt))
        cur.mergeCharFormat(cf)
...
"""
    ...
    def reload_source(self):
        if self.data_source["selected"] == "file":
            ...
            try:
                result = urllib.request.urlopen(self.data_source["source"]).read().decode("utf-8")
...
Do you want to use the text from the textbox instead of the original text?"""
    ...
    def btn_tag_remove_click(self):
        ...
        tttx = txt[text_dict["start"]:text_dict["end"]]
        if len(tttx) < 2000:
            print (tttx)
    ...
    def refresh_tree(self, set_current_segment_name: str = None):
        ...
        for i in range(len(top_segments)):
            self._add_tree_items(self.tree_seg.topLevelItem(i))
    ...
    def _add_tree_items(self, item: QTreeWidgetItem):
        ...
        for child in children:
            ...
            if self.script.get_segment_children(child, names_only=True):
                self._add_tree_items(child_item)
    ...
    def _resize_me(self):
        ...
        self.lbl_data_src.resize(self.frm_data_src.contentsRect().width() - self.lbl_data_src.pos().x() - 10, self.lbl_data_src.height())
